...
void PS1()
{
	...
	char* cwd;
        int usr_name_stat, host_name_stat;
        char buff[PATH_MAX + 1];
        char buff_l[LOGIN_NAME_MAX + 1];
        char buff_h[HOST_NAME_MAX + 1];

        cwd = getcwd( buff, PATH_MAX + 1 );
        usr_name_stat = getlogin_r( buff_l, LOGIN_NAME_MAX + 1);
        host_name_stat = gethostname( buff_h, HOST_NAME_MAX + 1);

        printf("<%s@%s:%s> ", buff_l, buff_h, cwd);
...
int main()
{
	...
	int in,out;
	pid_t pid,pid1;
	char *par[1000],exe[10000],pth[1000],*com[1000],cmd2[1000],*cmds[1000];
	...
	int i,n,j,background,sig,jno,status,ncom,k,x;
	//signal handling
	...
	while(strcmp(command,"quit")!=0)
	{
		...
		for(x=0;x<k;x++)
		{
			...
			if(piper==1)
			{
				piped(command);	//function to execute piped commands
				continue;
			}
			ipfile=malloc(sizeof(char));
			opfile=malloc(sizeof(char));
			strcpy(ipfile,getin(command));
			strcpy(opfile,getout(command));
			sscanf(command,"\n%[^<>]",cmd2);
			strcpy(command,cmd2);
			cmd1=no_white(command);
			n=split(command," \t\n",par);
			if(strcmp(par[n-1],"&")==0)
			{
				n--;
